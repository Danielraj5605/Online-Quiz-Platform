...
exports.updateUserXP = exports.getUserStats = exports.checkAndAwardBadges = exports.initializeBadges = exports.calculateTier = exports.calculateXP = void 0;
const db_1 = __importDefault(require("../db"));
// XP calculation: base XP + bonus for score + time bonus
const calculateXP = (score, total, timeTaken, timeLimit) => {
    const baseXP = 50;
...
exports.initializeBadges = initializeBadges;
// Check and award badges after quiz completion
const checkAndAwardBadges = async (userId, score, total, timeTaken, timeLimit, xpEarned) => {
    const newBadges = [];
    // Get user stats
    const userStats = await (0, exports.getUserStats)(userId);
    ...
    for (const badge of badges) {
        ...
        if (criteria.quizzesCompleted && userStats.totalQuizzes >= criteria.quizzesCompleted)
            earned = true;
        if (criteria.perfectScores && score === total && !earned)
            earned = true;
        if (criteria.speedBonuses && timeTaken < timeLimit * 0.25 && !earned)
            earned = true;
        if (criteria.xpInOneQuiz && xpEarned >= criteria.xpInOneQuiz && !earned)
            earned = true;
        if (criteria.totalXP && userStats.totalXP + xpEarned >= criteria.totalXP && !earned)
            earned = true;
        // Check time-based badges
        const now = new Date();
        const hour = now.getHours();
        if (criteria.earlyCompletions && hour < 12 && !earned)
            earned = true;
        if (criteria.lateCompletions && hour >= 22 && !earned)
            earned = true;
        // Check daily streak (simplified - just check if completed today)
        if (criteria.dailyStreak) {
            const todayStart = new Date(now.setHours(0, 0, 0, 0));
            const todayEnd = new Date(now.setHours(23, 59, 59, 999));
            const completedToday = await db_1.default.result.count({
                where: {
                    userId,
                    createdAt: { gte: todayStart, lte: todayEnd },
                },
            });
            if (completedToday >= criteria.dailyStreak)
                earned = true;
        }
        if (earned) {
...
const getUserStats = async (userId) => {
    const [totalQuizzes, totalXP, results] = await Promise.all([
        ...
        db_1.default.user.findUnique({ where: { id: userId }, select: { xp: true } }),
        db_1.default.result.findMany({
            where: { userId },
...
const updateUserXP = async (userId, xpEarned) => {
    const user = await db_1.default.user.findUnique({ where: { id: userId } });
    if (!user)
        return;
    const newXP = user.xp + xpEarned;
    const newTier = (0, exports.calculateTier)(newXP);
