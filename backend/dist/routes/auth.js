...
router.post('/register', (0, validate_1.validateBody)(auth_2.registerSchema), async (req, res) => {
    try {
...
router.post('/login', (0, validate_1.validateBody)(auth_2.loginSchema), async (req, res) => {
    try {
        const { email, password } = req.body;
        const user = await db_1.default.user.findFirst({ where: { email } });
        if (!user) {
...
// Forgot password
router.post('/forgot-password', (0, validate_1.validateBody)(auth_2.forgotPasswordSchema), async (req, res) => {
    try {
...
// Reset password
router.post('/reset-password', (0, validate_1.validateBody)(auth_2.resetPasswordSchema), async (req, res) => {
    try {
...
// Update profile
router.put('/profile', auth_1.authenticate, (0, validate_1.validateBody)(auth_2.profileUpdateSchema), async (req, res) => {
    try {
        ...
        if (username) {
            const exists = await db_1.default.user.findFirst({ where: { username, NOT: { id: userId } } });
            if (exists) {
        ...
        if (email) {
            const exists = await db_1.default.user.findFirst({ where: { email, NOT: { id: userId } } });
            if (exists) {
...
// Change password
router.put('/password', auth_1.authenticate, (0, validate_1.validateBody)(auth_2.passwordChangeSchema), async (req, res) => {
    try {
        ...
        const matches = await bcrypt_1.default.compare(currentPassword, user.password);
        if (!matches) {
